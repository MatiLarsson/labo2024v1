...
# Parametros asignados a Matias Larsson:
...
# saco la columna clase_ternaria
...
for (arbolito in 1:PARAM$num_trees_max) {
  # eligo aleatoriamente los campos a utilizar
  ...
  * PARAM$feature_fraction)
  ...
  # Para cada arbolito, acumulo la probabilidad
  # en la columna prob_acumulada de dapply
  # la probabilidad acumulada es la suma de las probabilidades de cada arbolito
  dapply[, prob_acumulada := prob_acumulada + prediccion[, "BAJA+2"]]

  if (arbolito %in% grabar) {
    # Genero la entrega para Kaggle
    # el umbral de corte es 1/40 del numero de arboles que ya he generado
    # dado que estoy acumulando la probabilidad de cada arbolito
    umbral_corte <- (1 / 40) * arbolito
    # a cada cliente del set de test
    # le asigno la clase 1 si la probabilidad acumulada
    # es mayor al umbral de corte y 0 en caso contrario
    ...
    nom_arch <- paste0(
      ...
      sprintf("%.3d", arbolito), "_Eli", # para que tenga ceros adelante
      ".csv"
    )
    fwrite(entrega,
      file = nom_arch,
...
setwd("~/labo2024v1/src/ArbolesAzarosos/") # Establezco el Working Directory
source("twilio.r")
wp("Arbolitos G estan listos!")