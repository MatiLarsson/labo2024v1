...
TS_strategy_guantesblancos_202109 <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$final_train <- c(202107, 202106, 202105, 202104, 202103, 202102, 202101, 202012, 202011, 202010, 202009, 202008, 202007, 202002, 202001, 201912, 201911, 201910, 201909, 201908, 201907, 201906, 201905, 201904, 201903)


  param_local$train$training <- c(202105, 202104, 202103, 202102, 202101, 202012, 202011, 202010, 202009, 202008, 202007, 202002, 202001, 201912, 201911, 201910, 201909, 201908, 201907, 201906, 201905, 201904, 201903, 201902, 201901)
...
HT_tuning_guantesblancos <- function( pmyexp, pinputexps, pserver="local")
{
  ...
  param_local$lgb_param <- list(
    ...
    lambda_l1 = c(0.0, 20.0), # lambda_l1 >= 0.0
    lambda_l2 = c(0.0, 20.0), # lambda_l2 >= 0.0
    ...
    is_unbalance = FALSE, #
    scale_pos_weight = c(1.0, 2.0), # scale_pos_weight > 0.0

    drop_rate = 0.1, # 0.0 < neg_bagging_fraction <= 1.0
    max_drop = 50, # <=0 means no limit
    skip_drop = c(0.0, 1.0), # 0.0 <= skip_drop <= 1.0

    extra_trees = TRUE,
    # White Gloves Bayesian Optimization, with a happy narrow exploration
    learning_rate = c( 0.015, 0.8 ),
